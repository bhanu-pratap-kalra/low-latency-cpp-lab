...
int main()
{
	...
	//The same pool implementation can noe manage another type
	
	MemoryPool<int> int_memory_pool{2};

	int* first_int = int_memory_pool.acquire();

	if (first_int != nullptr)
	{
		*first_int = 100;

		std::cout << "First int value from int memory pool = " << *first_int << std::endl;

		int_memory_pool.release(first_int);
	}
